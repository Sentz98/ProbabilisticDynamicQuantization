...
class TestSettings(ABC):
    ...
    def __init__(self,
        ...
        self.dynamic = testtype.split("_")[-1] == "dyn"
        self.test_type = testtype.split("_")[0] if self.dynamic else testtype
        ...
        self.output_table_file = self.pregenerated_data_dir + self.testdataset + '/' + 'output.txt'
        self.scales_table_file = self.pregenerated_data_dir + self.testdataset + '/' + 'scales.txt'
    ...
    def get_randomized_data(
        ...
        mean=0.0,        # New: Explicit mean for Gaussian
        stddev=1.0       # New: Explicit stddev for Gaussian
    ):
        # Handle defaults for min/max
        minrange = self.mins if minrange is None else minrange
        maxrange = self.maxs if maxrange is None else maxrange
        ...
        if not os.path.exists(npfile) or regenerate:
            ...
            if distribution == 'gaussian':
                data = tf.Variable(tf.random.normal(dims, mean=mean, stddev=stddev, dtype=tf.float32))
                
                data = tf.clip_by_value(data, minrange, maxrange)
                data = np.around(data.numpy(), decimals)
                data = tf.convert_to_tensor(data)
            else:
                # Original uniform logic
                if decimals == 0:
                    data = tf.Variable(tf.random.uniform(dims, minval=minrange, maxval=maxrange, dtype=tf.int64))
                    data = tf.cast(data, tf.float32)
                else:
                    data = tf.Variable(tf.random.uniform(dims, minval=minrange, maxval=maxrange, dtype=tf.float32))
                    data = np.around(data.numpy(), decimals)
                    data = tf.convert_to_tensor(data)
            print(f"Saving data to {npfile}")
            self.save_multiple_dim_array_in_txt(npfile, data.numpy())
        else:
            print(f"Loading data from {npfile}")
    ...
    def generate_c_array(self, name, array, datatype="int8_t", const="const ", pack=False, stats=False) -> None:
        w = None
        mean = None
        std = None
        ...
        if type(array) is list:
            ...
            w = array
            if stats:
                mean = np.mean(w, axis=tuple(range(1, w.ndim))).astype(int)
                std = np.std(w, axis=tuple(range(1, w.ndim))).astype(int)
                stat_size = w.shape[0]
        ...
        with open(filepath, "w+") as f:
            ...
            if size > 0:
                if name == "output_mult":
                    f.write(const + datatype + " " + self.testdataset + '_' + name + "[%d] ={ " % size)
                else:
            ...
            if stats:
                def write_array_to_file(array, array_name, size, file):
                    file.write("\n")
                    file.write(f"{const}{datatype} {self.testdataset}_{array_name}[{size}] = {{\n")
                    file.write(",\n".join(f"  {array[i]}" for i in range(size - 1)))
                    file.write(f",\n  {array[size - 1]}\n}};\n")

                # Write mean array
                mean_name = f"{name}_mean"
                write_array_to_file(mean, mean_name, stat_size, f)

                # Write std array
                std_name = f"{name}_std"
                write_array_to_file(std, std_name, stat_size, f)
    ...
    def get_calib_data_func(self, n_inputs, shape, ones=True):
            ...
            if n_inputs > 0:
                for i in range(n_inputs):
                    if ones:
                        ...
                        data = tf.Variable(tf.random.uniform(shape, dtype=tf.float32))
                        representative_testsets.append(np.around(data.numpy(), decimals=1))
    ...
    def get_calib_dynamic_func(self, input_data):

        def representative_data_gen():
            representative_testsets = []
            if input_data is not None:
                representative_testsets.append(input_data.numpy())
                yield representative_testsets
            else:
                raise RuntimeError("cosa stai passando??")
    ...
    def convert_model(self, model, inttype, dataset_shape=None, int16x8_int32bias=False, input_data=None):
        ...
        if input_data is None:
            representative_dataset = self.get_calib_data_func(n_inputs, representative_dataset_shape, ones=False)
        else:
            representative_dataset = self.get_calib_dynamic_func(input_data)
