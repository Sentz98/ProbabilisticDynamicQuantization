...
class ConvSettings(TestSettings):
    ...
    def generate_quantize_per_channel_multiplier(self):
        ...
        if self.dynamic:
            with open(self.scales_table_file,"w") as f:
                np.savetxt(f,
                        np.array([self.input_scale]), 
                        header='Input_scale', 
                        delimiter=',')
                np.savetxt(f,
                        np.array([self.input_zero_point]), 
                        header='Input_zp', 
                        delimiter=',')
                np.savetxt(f,
                        self.scaling_factors.reshape(-1, self.scaling_factors.shape[-1]), 
                        header='Weight_scale', 
                        delimiter=',')
                np.savetxt(f,
                        np.array([self.output_scale]), 
                        header='Output_scale', 
                        delimiter=',')
                np.savetxt(f,
                        np.array([self.output_zero_point]), 
                        header='Output_zp', 
                        delimiter=',')
    ...
    def generate_data(self, input_data=None, weights=None, biases=None) -> None:
        ...
        if self.int4_weights:
            ...
            if self.test_type == 'conv':
                ...
                if self.dynamic:
                    ...
                    self.save_multiple_dim_array_in_txt(self.output_table_file, output_data)
            ...
            self.convert_model(model, inttype, int16x8_int32bias=self.int16xint8_int32, input_data=input_data)
        ...
        self.generate_c_array(
            self.weight_data_file_prefix, interpreter.get_tensor(filter_layer['index']), pack=self.int4_weights, stats=self.dynamic)
